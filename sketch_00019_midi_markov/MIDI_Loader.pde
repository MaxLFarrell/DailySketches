...
*/
  ...
  void toTxt(){
    String notes = "";
    for (int it=0;it<numTracks();it++){
      for (Note n:trackAsArrayList(it)){
        notes += n.start + " " + n.duration + " " + n.channel + " " + n.velocity + " " + n.pitch + ".\n"; 
      }
    }
    saveStrings("txt/" + filename + ".txt", new String[]{notes});
  }
}
