// Code adapted from here: https://www.openprocessing.org/sketch/92464

List<PShape> icosa(Icosahedron ico, int size) {
  PVector f1 = new PVector();
  PVector f2 = new PVector();
  PVector f3 = new PVector();
  
  List<PShape> res = new ArrayList<PShape>(); 
  for ( int i = 0; i < ico.vertexList.size(); i+=9 ) {
    f1.x = ico.vertexList.get(i) * size;
    f1.y = ico.vertexList.get(i+1) * size;
    f1.z = ico.vertexList.get(i+2) * size;
    f2.x = ico.vertexList.get(i+3) * size;
    f2.y = ico.vertexList.get(i+4) * size;
    f2.z = ico.vertexList.get(i+5) * size;
    f3.x = ico.vertexList.get(i+6) * size;
    f3.y = ico.vertexList.get(i+7) * size;
    f3.z = ico.vertexList.get(i+8) * size;
    PShape tmp = createShape();
    tmp.beginShape();
    tmp.vertex( f1.x, f1.y, f1.z );
    tmp.vertex( f2.x, f2.y, f2.z );
    tmp.vertex( f3.x, f3.y, f3.z );
    tmp.endShape( CLOSE );
    res.add(tmp);
  }
  return res;
...
public class Icosahedron {
  ...
  public Icosahedron(int subdivisions, int numPoints) {
    subdivision = subdivisions;
    // Iterate over points
